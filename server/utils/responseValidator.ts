 ...
 * Rules:
 * 1. traditions array non-empty
 * 2. each tradition has position and key_texts
 */
export function validateGeneralQaResponse(response: GeneralQaResponse): ValidationResult {
  const errors: string[] = []

  if (!Array.isArray(response.traditions) || response.traditions.length === 0) {
    errors.push('traditions must be a non-empty array')
  } else {
 ...
 * Rules:
 * 1. traditions array non-empty, each tradition has position and key_texts
 * 2. Heuristic check: flag if response starts with direct "Yes," or "No," verdict
 */
export function validateMoralQuestionResponse(response: MoralQuestionResponse): ValidationResult {
  ...
  // Simple heuristic check: ensure response does NOT start with direct "Yes," or "No," verdict language
  ...
  if (/^(yes|no)[.,!\s]/i.test(firstTraditionPosition) || /^(yes|no)[.,!\s]/i.test(response.divergenceNote ?? '')) {
    console.warn('[MoralQuestionValidator] Direct yes/no verdict language detected at response opening:', {
